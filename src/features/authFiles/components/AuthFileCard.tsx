...
import type { AuthFilesSweepFinding } from '@/features/authFiles/automation';
...
export type AuthFileCardProps = {
  ...
  sweepFinding?: AuthFilesSweepFinding | null;
  keyStats: KeyStats;
    ...
    >
      <div className={styles.fileCardLayout}>
        <div className={styles.fileCardMain}>
          ...
          {sweepFinding &&
            (sweepFinding.reasons.includes('status-401') ||
              sweepFinding.reasons.includes('quota-401')) && (
              <div className={styles.quotaError}>{t('auth_files.sweep_card_401')}</div>
            )}

          {sweepFinding && sweepFinding.reasons.includes('weekly-low') && (
            <div className={styles.quotaWarning}>
              {t('auth_files.sweep_card_weekly_low', {
                percent:
                  sweepFinding.weeklyRemainingPercent === null
                    ? '--'
                    : Math.round(sweepFinding.weeklyRemainingPercent),
              })}
            </div>
          )}

...
import { useAuthFilesSweep } from '@/features/authFiles/hooks/useAuthFilesSweep';
import {
  isAuthFilesSortMode,
...
import { AuthFilesSweepPanel } from '@/features/authFiles/components/AuthFilesSweepPanel';
...
export function AuthFilesPage() {
  ...
  const {
    ...
    result: sweepResult,
    findingsMap: sweepFindingsMap,
    nextRunAt: sweepNextRunAt,
    updateSettings: updateSweepSettings,
    runSweep,
  } = useAuthFilesSweep({
    files,
    loading,
  ...
  const sweepProblemNames = useMemo(
    () => new Set(Object.keys(sweepFindingsMap)),
    [sweepFindingsMap]
  );
  ...
  const filesMatchingProblemFilter = useMemo(
    () =>
      problemOnly
        ? files.filter(
            (file) => hasAuthFileStatusMessage(file) || sweepProblemNames.has(file.name)
          )
        : files,
    [files, problemOnly, sweepProblemNames]
  );
            ...
            <Button
              ...
              onClick={() =>
                handleDeleteAll({
                  filter,
                  problemOnly,
                  problemNames: sweepProblemNames,
      ...
      >
        ...
        <div className={styles.filterSection}>
          ...
          <AuthFilesSweepPanel
            ...
            onRun={() => void runSweep('manual')}
            onSettingsChange={updateSweepSettings}
          />
        </div>
          ...
          >
            {pageItems.map((file) => (
              <AuthFileCard
                ...
                sweepFinding={sweepFindingsMap[file.name] ?? null}
                keyStats={keyStats}
